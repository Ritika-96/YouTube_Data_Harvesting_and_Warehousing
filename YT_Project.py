...
st.set_page_config(layout='wide', page_title='Youtube Data Harvesting', page_icon='chart_with_upwards_trend')
...
col1, col2, col3 = st.tabs(['Data collection', 'Data migration', 'Data analysis'])
with col1:
    st.header(':violet[Data collection zone]')
    st.write(
        '(Note:- This zone **collects data** by using channel id and **stores it in the :green[MongoDB] database**.)')
    channel_id = st.text_input('**Enter a valid digit channel_id** and click the button below')
...
with col2:
    ...
    st.write('''(Note:- This zone migrates specific channel data to **:blue[MySQL] database from :green[MongoDB] database** depending on your selection,
                if your required option is unavailable, go to data collection tab.)''')
    ...
    document_name = st.selectbox('**Select Channel name** and click the button below', options=document_names, key='document_names')
    ...
    if Migrate or st.session_state_migrate_sql:
        ...
        playlist_df.to_sql(name = 'playlist', con= engine, if_exists='append', index=False,
        ...
        # Commend data to SQL
        Comments_df.to_sql(name = 'comments', con= engine, if_exists='append', index=False,
...
with col3:
    st.header(':violet[Data Analysis zone]')
    st.write(
        '''(Note:- This zone performs the **Analysis of a collection of channels** depending on your question and provides an output in table format.)''')

    # Check available channel data
    Check_channel = st.checkbox('**Check the channels list available at the moment for analysis**')

    if Check_channel:
        # Create database connection
        engine = create_engine('mysql+mysqlconnector://root:Ritika1996@localhost/youtube_db', echo=False)
    ...
    connect_for_question = pymysql.connect(host='localhost', user='root', password='Ritika1996', db='youtube_db')
